...
- **语言扩展至 16 种**：i18n 新增 12 种主流语言——法语 / 德语 / 西班牙语 / 意大利语 / 葡萄牙语 / 俄语（欧洲）+ 韩语 / 泰语 / 越南语 / 印尼语 / 土耳其语 / 阿拉伯语（其它地区）。`T` 字典由 4 语言扩至 16 语言（每语言 21 键键集一致，两 Client 形态字典逐字相同），`LANG_KEYS`/`LANG_LABELS` 与下拉项同步；`detectLang` 扩展为按语言前缀映射（`fr`/`de`/`es`/`it`/`pt`/`ru`/`ko`/`th`/`vi`/`id`/`tr`/`ar` 及其区域变体命中各自语言，zh/ja/en 原规则不变，未知回退英文）；默认标的短名随新语言本地化（如 ^N225 → 닛케이 225 / نيكاي 225）。

### Fixed

- **语言下拉展开列表可读性**：⚙ 面板语言下拉在深色外观下展开列表为白底、未选中项灰字难以辨认；现给 `.shq-lang option` 显式深色文字（#111）+ 白底，深浅两套外观下未选中项均清晰可读（闭合态下拉本身样式不变）。
...
- **市场状态灯（收起态）**：胶囊标题内的状态点接入真实逻辑——任一自选市场处于当地交易时段（与休市自动降频共用 `anyMarketOpen` 判断）时显示绿色微光，全部休市时为红色；休市期间每 60 秒自动刷新一次判断（与轮询调度同拍），开盘即变绿，无需额外请求。

### Changed

- **收起态胶囊微调**：状态灯右侧留出 2px 空隙，胶囊宽度略有舒展、不再紧贴。
- **头部按钮统一**：⚙ / 展开 / 收起按钮改为 18×18px 固定方形 + flex 居中，字符在按钮内真正居中；收起键符号由全角破折号「—」改为数学减号「−」（U+2212），与展开「+」视觉成对；收起态头部右侧/顶部内边距与展开态统一（仅底部收紧），收起/展开切换时按钮位置像素级不动。
...
- **紧凑收起态（右缘锚定）**：窗口改为以**右缘锚定**（`pos.x` = 距视口右缘的 CSS `right`），收起/展开切换时右侧按钮位置不动、宽度向左自由伸缩；收起后收缩为内容自适应的小胶囊——标题为 CSS 绘制的柱状行情图标 + 占位状态点（无文字、不受语言标题长度影响），自动隐藏 ⚙ 设置键，仅保留展开按钮；展开态布局不变。

### Changed

- **设置面板分组与间距**：列表区 / 添加行 / 提示行 / 语言行 / 配色按钮 / 底部操作之间间距拉大（margin 10–12px 分级），新增语言下拉行；**编辑态面板加宽至 300px**（仅编辑时），长本地化文案（含英文）单行放得下、不再折行；配色按钮增加悬停说明（点击切换配色方案）；英文 hint 文案精简。
