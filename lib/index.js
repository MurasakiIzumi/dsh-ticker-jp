...
// VALUES identical to host.js (the two host halves only diverge in how they
// fetch, never in these strings or the RPC contract).
...
async function fetchOne(symbol) {
  ...
  try {
    ...
    if (!res.ok) throw new Error(MSG_HTTP(res.status))
      ...
      handler: async (req, res) => {
        try {
          ...
          const body = JSON.stringify(
            items.length
              ? { ok: true, items }
              : { ok: false, error: errors.length ? errors.join(MSG_JOIN) : MSG_NO_QUOTES }
          )
